...
export function SelectBank({ bankAccounts, onSelectBank, onAddBank }: SelectBankProps) {
  ...
  return (
    <div className="w-full">
      <h2 className="text-xl font-semibold mb-4">{tCrossborder("deposit.stasis.selectBank.heading")}</h2>
      {bankAccounts.length > 0 ? (
        <ul className="space-y-4">
          {bankAccounts.map((account) => (
            <li key={account.uuid} className="border p-4 rounded-lg bg-gray-50 hover:bg-gray-100 transition">
              <p className="text-sm font-medium text-gray-800 mb-2">{account.name}</p>
              <p className="text-xs text-gray-500 mb-1">
                {tCrossborder("deposit.stasis.selectBank.owner")} {account.holder_name}
              </p>
              <p className="text-xs text-gray-500 mb-1">
        ...
        <p className="text-gray-600">{tCrossborder("deposit.stasis.selectBank.noAccounts")}</p>
      )}
      <button
        ...
        onClick={onAddBank}
      >
        <IoAdd className="mr-2" /> {tCrossborder("deposit.stasis.selectBank.addButton")}
      </button>
    </div>
  );
}
