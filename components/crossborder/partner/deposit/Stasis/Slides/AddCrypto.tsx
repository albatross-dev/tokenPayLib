...
export function AddCrypto({
  ...
  return (
    <div className="w-full">
      <div className="mb-2">
        <label htmlFor="crypto_name" className="block text-sm font-medium text-gray-700">
      ...
      <LoadingButton isLoading={loadingState} active={!!newCryptoAccountName} onClick={onAddCryptoAccount}>
        <IoAdd className="mr-2" /> {tCrossborder("deposit.stasis.addCrypto.button")}
      </LoadingButton>

      {errors.cryptoAccount && <p className="text-red-500 text-sm mt-2">{errors.cryptoAccount}</p>}
    </div>
  );
}
