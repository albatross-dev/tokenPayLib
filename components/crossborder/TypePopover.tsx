...
const TypePopover: React.FC<TypePopoverProps> = ({ onSelect }) => {
  const [selectedType, setSelectedType] = useState<TransactionType>('all'); // default selected type
  ...
  const handleButtonClick = (value: TransactionType): void => {
              ...
              <div className="overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
                <div className="relative bg-white p-4">
                  {options.map(({ value, label }) => (
                    <button 
                      key={value} 
                      onClick={() => handleButtonClick(value as TransactionType)} 
