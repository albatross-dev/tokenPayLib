import { ConsoleTransport, Transport } from "../transport";
...
export class Reporter {
    ...
    private setupGlobalErrorHandler() {
        ...
        if (typeof window !== "undefined" && !window.onerror) {
            window.onerror = (errorMsg, url, lineNumber, column, errorObj) => {
                this.exception(
                    'An unhandeld error occured', {url, lineNumber, column }, errorObj)
        ...
        if (typeof window !== 'undefined' && !(<any>window).onunhandledrejection) {
            (<any>window).onunhandledrejection = (event: PromiseRejectionEvent) => {
                ...
                this.exception(`An unhandeld rejection occured`, undefined, event.reason)
    ...
    private buildExceptionObject(e: any): any {
        ...
        if (e.stack) { exceptionObject.stack = e.stack; } else { exceptionObject.e = e; }
        if (e.message) { exceptionObject.message = e.message; }
        if (e.name) { exceptionObject.name = e.name; }
        if (e.data) { exceptionObject.data = e.data; }
        if (e.inner) { exceptionObject.inner = this.buildExceptionObject(e.inner); }
    ...
    private stringifyData(data: any): string {
        ...
        switch (typeof actualData) {
            ...
            case "undefined":
                return "undefined";
            case "object":
                if((actualData instanceof RegExp) ||
