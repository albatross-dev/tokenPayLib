...
export async function fetchPaths(
  originToken: SimpleToken,
  activeChain: Chain,
  options: FetchPathsOptions = {}
): Promise<Pool[]> {
  try {
    if (!originToken?.id || !activeChain?.id) {
      console.warn("fetchPaths: missing originToken.id or activeChain.id");
      return [];
    }

    const limit = options.limit ?? 200;
    const cacheTtlMs = options.cacheTtlMs ?? 15000;
    const cacheKey = buildCacheKey(originToken, activeChain, limit);
